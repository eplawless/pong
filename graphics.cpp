...
bool Graphics::Initialize(uint32_t screenWidth, uint32_t screenHeight, HWND hwnd)
{
	...
	if (!m_playerPaddle.Initialize(pDevice) || !m_computerPaddle.Initialize(pDevice) || !m_ball.Initialize(pDevice))
	{
	...
	Input const &input)
{
	...
	m_ball.Update(usDeltaTime, m_playerPaddle, m_computerPaddle);
